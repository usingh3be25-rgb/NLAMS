...
# 1. GIS.js
gis_js = """// NLAMS GIS Command Center Component (Leaflet.js Delhi Map)
  ...
  useEffect(() => {
    ...
    parcelPolygons.forEach((poly) => {
        ...
        <div style="font-family: sans-serif; font-size: 11px; line-height: 1.4; min-width: 180px;">
          <div style="font-weight: bold; font-size: 13px; color: #1E293B; margin-bottom: 4px; display: flex; align-items: center; justify-content: space-between;">
            ...
            <span style="font-size: 10px; padding: 2px 6px; border-radius: 4px; background: ${poly.color}22; color: ${poly.color}; font-weight: bold;">
              ${pData.acquisitionStatus || 'Status'}
            </span>
          </div>
          ...
          <div style="margin-top: 8px; text-align: right;">
            <a href="javascript:void(0)" id="popup-btn-${poly.id}" style="color: #4F46E5; font-weight: bold; text-decoration: underline;">
              View Details →
            </a>
          </div>
        </div>
      ...
      pg.on('click', () => {
        ...
        setTimeout(() => {
          const btn = document.getElementById(`popup-btn-${poly.id}`);
          if (btn) {
            btn.onclick = () => {
...
with open(os.path.join(pages_dir, "GIS.js"), "w", encoding="utf-8") as f:
    f.write(gis_js)

print("Written GIS.js and Acquisition.js")
